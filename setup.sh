...
while getopts h:c:g:s:r:p:q: flag
do
    case "${flag}" in
        h) echo "$usage"; exit;;
...
gh secret set "CLUSTER_NAME" $cluster_name
gh secret set "RESOURCE_GROUP_NAME" $resource_group_name
gh secret set "STORAGE_ACCOUNT_NAME" $STORAGE_ACCOUNT_NAME
gh secret set "STORAGE_CONTAINER_NAME" $CONTAINER_NAME
gh secret set "STORAGE_ACCESS_KEY" $ACCOUNT_KEY
gh secret set "ARM_CLIENT_ID" $ARM_CLIENT_ID
gh secret set "ARM_CLIENT_SECRET" $ARM_CLIENT_SECRET
gh secret set "ARM_SUBSCRIPTION_ID" $subscription
gh secret set "ARM_TENANT_ID" $ARM_TENANT_ID
